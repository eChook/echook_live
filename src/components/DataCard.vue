...
<template>
  <div :class="[
    'group flex-shrink-0 min-w-20 md:min-w-32 p-2 md:p-3 bg-white dark:bg-neutral-800 rounded-lg border border-zinc-200 dark:border-neutral-700 shadow flex flex-col items-center justify-center space-y-0.5 md:space-y-1 transition-all duration-200 cursor-grab active:cursor-grabbing hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 hover:-translate-y-0.5',
    ...
    <span
      :class="['text-lg md:text-2xl font-mono font-bold transition-colors duration-500', stale ? 'text-zinc-400 dark:text-gray-500' : 'text-zinc-900 dark:text-white']">{{
        displayValue }}</span>
  </div>
</template>
