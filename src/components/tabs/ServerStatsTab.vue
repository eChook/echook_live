<template>
    <div class="h-full flex flex-col space-y-6">
        ...
        <div v-else class="space-y-6 overflow-y-auto px-8 pb-10">
            <!-- Top Cards -->
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-neutral-800 rounded-lg p-4 border border-neutral-700">
                    ...
                    <div class="text-2xl font-mono text-primary mt-1">{{ adminStore.serverStats.current?.activeCars || 0
                        }}</div>
                </div>
                <!-- Spectators -->
                <div class="bg-neutral-800 rounded-lg p-4 border border-neutral-700">
                    <div class="text-gray-400 text-xs uppercase font-bold tracking-wider">Spectators</div>
                    <div class="flex items-end space-x-2 mt-1">
                        <span class="text-2xl font-mono text-white">{{ (adminStore.serverStats.current?.publicSpectators
                            || 0) + (adminStore.serverStats.current?.privateSpectators || 0) }}</span>
                        <span class="text-xs text-gray-500 mb-1">
                            (Pub: {{ adminStore.serverStats.current?.publicSpectators || 0 }} / Priv: {{
                                adminStore.serverStats.current?.privateSpectators || 0 }})
                        </span>
                    </div>
                </div>
                <!-- CPU Load -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- Memory -->
                <div class="bg-neutral-800 rounded-lg p-4 border border-neutral-700 flex justify-between items-center">
                    <div>
            ...
            <div class="bg-neutral-800 rounded-lg border border-neutral-700 overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-neutral-700 font-mono text-sm">
                        ...
                        <tr v-if="Object.keys(spectatorDistribution).length === 0">
                            <td colspan="2" class="px-6 py-4 text-center text-gray-500 italic">No active spectators</td>
                        </tr>
                    </tbody>
                </table>
            </div>
