...
<template>
    <div class="h-full flex overflow-hidden">
        ...
        <div class="w-1/3 min-w-[300px] border-r border-zinc-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto p-2 space-y-2">
                <div v-for="track in adminStore.tracks" :key="track._id" @click="selectedTrackId = track._id"
                    ...
                    <div>
                        ...
                        <div class="text-xs text-zinc-500 dark:text-gray-500 font-mono">
                            {{ track.latMin.toFixed(4) }}, {{ track.lonMin.toFixed(4) }}
                        </div>
                    </div>

                    <button @click.stop="deleteTrack(track._id)"
                        class="text-zinc-400 dark:text-neutral-500 hover:text-red-600 dark:hover:text-red-500 opacity-0 group-hover:opacity-100 transition p-2">
            ...
            <div v-if="selectedTrack" class="p-4 bg-zinc-50 dark:bg-neutral-900 border-t border-zinc-200 dark:border-neutral-700 space-y-3">
                <div>
                    <label class="block text-xs uppercase text-zinc-600 dark:text-gray-400 font-bold mb-1">Track Name</label>
                    <input v-model="selectedTrack.name" type="text"
                        class="w-full bg-white dark:bg-neutral-800 border border-zinc-300 dark:border-neutral-700 rounded p-2 text-zinc-900 dark:text-white focus:border-primary outline-none">
                </div>

                <div class="grid grid-cols-2 gap-2 text-xs font-mono text-zinc-600 dark:text-gray-400">
                    <div class="bg-zinc-100 dark:bg-neutral-800 p-2 rounded">Lat Min: {{ selectedTrack.latMin.toFixed(5) }}</div>
                    <div class="bg-zinc-100 dark:bg-neutral-800 p-2 rounded">Lat Max: {{ selectedTrack.latMax.toFixed(5) }}</div>
                    <div class="bg-zinc-100 dark:bg-neutral-800 p-2 rounded">Lon Min: {{ selectedTrack.lonMin.toFixed(5) }}</div>
                    <div class="bg-zinc-100 dark:bg-neutral-800 p-2 rounded">Lon Max: {{ selectedTrack.lonMax.toFixed(5) }}</div>
                </div>
        ...
        <div class="flex-1 relative bg-zinc-200 dark:bg-neutral-900">
            ...
            <div v-if="!selectedTrack"
                class="absolute top-4 left-4 z-[500] bg-white/90 dark:bg-neutral-900/80 backdrop-blur p-4 rounded text-zinc-900 dark:text-white shadow-xl pointer-events-none border border-zinc-200 dark:border-neutral-700">
                <h2 class="font-bold">Map Editor</h2>
                <p class="text-sm text-zinc-600 dark:text-gray-300">Select a map from the left to edit boundaries, or create a new one.</p>
            </div>
        </div>
    </div>
</template>
