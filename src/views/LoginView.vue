  ...
  <div class="flex flex-col min-h-screen bg-zinc-100 dark:bg-neutral-900" :style="{
    ...
    <div class="flex-1 flex flex-col items-center justify-center pt-32 px-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
        ...
        <!-- Login Panel -->
        <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-2xl border border-zinc-200 dark:border-neutral-700 p-8">
          <h2 class="text-3xl font-bold mb-6 text-center text-primary">Login</h2>

          <div v-if="error"
            class="mb-4 p-3 bg-red-100 dark:bg-red-900/50 border border-red-400 dark:border-red-500 text-red-800 dark:text-red-200 rounded text-sm text-center">
            {{ error }}
          </div>

          <div v-if="auth.isAuthenticated" class="text-center py-8">
            <p class="text-zinc-600 dark:text-gray-300 mb-6">
            ...
            <button @click="auth.logout()" class="mt-4 text-sm text-zinc-500 dark:text-gray-500 hover:text-zinc-900 dark:hover:text-white transition">
              Log Out
            </button>
          </div>
          ...
          <form v-else @submit.prevent="handleLogin" class="space-y-5">
            <div>
              <label class="block mb-2 text-sm font-medium text-zinc-600 dark:text-gray-400">Car Name</label>
              <input v-model="car" type="text" required
                class="w-full bg-white dark:bg-neutral-900 border border-zinc-300 dark:border-neutral-600 rounded-lg p-3 text-zinc-900 dark:text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition"
                placeholder="Enter your Car Name">
            </div>
            <div>
              <label class="block mb-2 text-sm font-medium text-zinc-600 dark:text-gray-400">Password</label>
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-2xl border border-zinc-200 dark:border-neutral-700 p-8 flex flex-col">
          ...
          <div class="flex-1 overflow-y-auto space-y-3 custom-scrollbar">
            <div v-if="spectatorStore.activeTracks.length === 0"
              class="text-center py-10 text-zinc-500 dark:text-gray-500 border-2 border-dashed border-zinc-300 dark:border-neutral-700 rounded-lg">
              No races ongoing
            </div>
          ...
          <div class="mt-4 pt-4 border-t border-zinc-200 dark:border-neutral-700 text-xs text-zinc-500 dark:text-gray-500 text-center leading-relaxed">
      ...
      <!-- What's New Section -->
      <div class="mt-8 bg-white dark:bg-neutral-800 rounded-xl shadow-2xl border border-zinc-200 dark:border-neutral-700 p-8 w-full max-w-4xl">
        <h2 class="text-2xl font-bold text-zinc-900 dark:text-white mb-6 border-b border-zinc-200 dark:border-neutral-700 pb-4">What's New?</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-zinc-700 dark:text-gray-300 text-sm">
          <ul class="space-y-3">
